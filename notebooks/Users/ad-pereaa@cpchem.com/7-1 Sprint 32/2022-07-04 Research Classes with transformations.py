...
class MLRScaler(BaseEstimator, TransformerMixin):
  ...
  def transform(self, X, y = None, inverse = False):
    X = cp.deepcopy(X)
...
# MAGIC %md
# MAGIC #### class for pipeline
# MAGIC This class will allow for pipeline scale of Y values. 
...
class MLRModel(Pipeline):
  ...
  def __init__(
    ...
    self.scale_type = scale_type
    self.scale_scope = scale_scope
    self.scale_category_cols = scale_category_cols
    ...
    steps = [
      (
        ...
        MLRScaler(
          ...
          categories = self.scale_category_cols
        )
      ),
      (
        'model_prep', 
        ColumnTransformer(
          [
             ( 
               ...
               Pipeline(
                 steps = [
                   ('selector', MLRColumnSelector(cols_to_keep = self.ohe_cols)),
             ...
             (
               ...
               Pipeline(
                 steps = [
                   ('selector', MLRColumnSelector(cols_to_keep = self.scale_cols)),
                   ('pca', 'passthrough' if self.pca is None else self.pca)
                 ]
               ),
               self.scale_cols
             )
    ...
    return super().__init__(steps = steps, verbose = verbose)
  
  def fit(self, X, y = None):
    
    self.y_transformer_ = MLRScaler(
      scale_cols = ['y'], 
      scale_type = self.scale_type, 
      scale_scope = self.scale_scope, 
      categories = self.scale_category_cols
    ...
    df_y = None
    if (self.scale_scope == 'categorical'):
      df_y = X.loc[:, self.scale_category_cols]
      df_y['y'] = y
    else: df_y = pd.DataFrame({'y': y})
    
    df_y = self.y_transformer_.fit_transform(df_y)
    
    return super().fit(X, df_y['y'])
  ...
  def predict(self, X): 
    ...
    df_y = self.y_transformer_.inverse_transform(df_y)
    return df_y['y']

# COMMAND ----------

# MAGIC %md
# MAGIC #### preprocessing testing
...
tx_scl_u = MLRScaler(scale_type = 'robust', scale_scope = 'universal', categories = ['Extruder', 'Test Type'])
df_tx_scl_u = tx_scl_u.fit_transform(df_tx_dt)

tx_scl_c = MLRScaler(scale_type = 'robust', scale_scope = 'categorical', categories = ['Extruder', 'Test Type'])
...
df_trn = df_raw[df_raw.Extruder.eq(p_extruder) & df_raw[p_date_col].lt('2022-01-01')]
df_tst = df_raw[df_raw.Extruder.eq(p_extruder) & df_raw[p_date_col].ge('2022-01-01')]
...
# MAGIC %md
# MAGIC #### pipeline - model

# COMMAND ----------

# Determine the numeric columns to include. 
X_scale_group = ['Extruder', 'Test Type']
X_ohe_cols    = ['Extruder', 'Test Type']
X_scale_cols  = ['Extruder Fluff Feed Rate', 'Gear Pump Bearing 1 Temp', 'Gear Pump Bearing 2 Temp', 'Gear Pump Bearing 3 Temp', 'Gear Pump Bearing 4 Temp', 'Screenpack dP', 'Zone 3 Temp', 'Zone 10 Temp', 'y_prev', 'dT']
X_diff_cols = []
for c in X_scale_cols:
  dc = f'd({c})'
  if dc in df_trn.columns:
    X_diff_cols.append(dc)
X_scale_cols += X_diff_cols

df_trn[X_ohe_cols + X_scale_cols]

# COMMAND ----------

# class MLRModel(scale_cols, scale_type = 'standard', scale_scope = 'universal', scale_category_cols = None, ohe_cols = None, pca = None, verbose = False):
model = MLRModel(
  scale_cols = X_scale_cols,
  scale_type = 'standard', 
  scale_scope = 'categorical',
  scale_category_cols = X_scale_group,
  ohe_cols = X_ohe_cols,
  pca = None,
  regressor = XGBRegressor(subsample = 0.7, n_estimators = 500, min_child_weight = 9, max_depth = 9, learning_rate = 0.1, metric = 'mape'),
  verbose = True
)
...
model.fit(df_trn.loc[:, ~df_trn.columns.isin(['y'])], df_trn['y'])

# COMMAND ----------

pred_y = model.predict(df_tst.loc[:, ~df_tst.columns.isin(['y'])])
df_pred = df_tst.loc[:, [p_date_col, 'Extruder', p_resin_col, 'Test Type', 'dT', 'y']]
df_pred['y\''] = pred_y
df_pred
...
with parallel_backend('spark', n_jobs = 32):
  cv_model.fit(df_trn.loc[:, ~df_trn.columns.isin(['y'])], df_trn['y'])

# COMMAND ----------

pd.DataFrame(cv_model.cv_results_).T
